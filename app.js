...
function rankFromXP(xp=0){
  ...
  return sorted.filter(r=>xp>=r.min).at(-1)?.name || sorted[0].name;
}
function addXP(amount=0, source='generic'){
  const gain=Math.max(0, Number(amount)||0);
  S.rpg.xp=(S.rpg.xp||0)+gain;
  S.rpg.level=Math.max(1,Math.floor((S.rpg.xp||0)/120)+1);
  S.rpg.rank=rankFromXP(S.rpg.xp||0);
  if(gain>0){
    S.rpg.combo=Math.min(999,(S.rpg.combo||0)+1);
    S.streakLog[todayKey()]=true;
  }
  return {gain,source};
}
function adjustIntegrity(delta=0){ S.rpg.integrity=Math.max(0,Math.min(100,(S.rpg.integrity||100)+(Number(delta)||0))); }
function setLastAction(action){ S.lastAction=action||null; }
function undoLastAction(){
  const a=S.lastAction;
  if(!a) return showToast('Nada para desfazer');
  if(a.type==='trainSet' && a.entry){
    const i=S.training.history.findIndex(x=>x===a.entry || (x.date===a.entry.date && x.exercise===a.entry.exercise));
    if(i>=0) S.training.history.splice(i,1);
  }
  if(a.type==='studyAdd'){
    const i=S.study.history.findIndex(x=>x.date===a.date&&x.minutes===a.minutes&&x.topic===a.topic);
    if(i>=0) S.study.history.splice(i,1);
  }
  if(a.type==='taskAdd' && a.entry){
    const k=todayKey();
    const arr=S.tasks.byDate[k]||[];
    const i=arr.findIndex(x=>x.title===a.entry.title&&x.time===a.entry.time);
...
function ensureDietToday(){
  const k=todayKey();
  let d=S.diet.history.find(x=>x.date===k);
  if(!d){ d={date:k,meals:{cafe:0,almoco:0,lanche:0,jantar:0},water:0}; S.diet.history.push(d); }
  d.meals=d.meals||{cafe:0,almoco:0,lanche:0,jantar:0};
  return d;
}
function sumDiet(day){
  ...
  slots.forEach(slot=>{ const idx=Number(meals[slot]||0); const m=(MEALS[slot]||[])[idx]; if(!m) return; total.kcal+=m.kcal; total.p+=m.p; total.c+=m.c; total.g+=m.g; });
  return total;
}
function todayProgress(){
  const k=todayKey();
  const pillars=[
    (S.study.history||[]).some(x=>x.date===k),
    (S.training.history||[]).some(x=>x.date===k),
    !!S.bibleLog[k],
    ((S.tasks.byDate||{})[k]||[]).length===0,
    !(S.diary.history||[]).find(x=>x.date===k)?.porn
  ];
  const done=pillars.filter(Boolean).length;
  return {done,total:pillars.length,pct:Math.round(done/pillars.length*100)};
}
function timeToMinutes(hhmm='00:00'){ const [h,m]=String(hhmm).split(':').map(Number); return (h||0)*60+(m||0); }
function currentMissionWindow(now=new Date()){
  const mins=now.getHours()*60+now.getMinutes();
  const w=S.windows||{};
  const windows=[
    {id:'wake',tab:'PROTO',start:timeToMinutes(w.wake),end:timeToMinutes(w.morningEnd),title:'Rotina da manhã'},
    {id:'study',tab:'ESTUDO',start:timeToMinutes(w.studyStart),end:timeToMinutes(w.studyEnd),title:'Bloco de estudo'},
    {id:'work',tab:'OPS',start:timeToMinutes(w.workStart),end:timeToMinutes(w.workEnd),title:'Projetos e execução'},
    {id:'train',tab:'TREINO',start:timeToMinutes(w.trainStart),end:timeToMinutes(w.trainEnd),title:'Treino principal'},
    {id:'night',tab:'DIARIO',start:timeToMinutes(w.nightStart),end:timeToMinutes(w.sleep),title:'Fechamento do dia'}
  ];
  const hit=windows.find(win=>mins>=win.start&&mins<win.end);
  return hit || {id:'sleep',tab:'DASH',title:'Sono e recuperação'};
}
function refreshHUD(){
  const mission=currentMissionWindow();
  const day=Math.max(1,Math.floor((Date.now()-new Date(S.campaignStart||Date.now()).getTime())/86400000)+1);
  const streakDays=Object.keys(S.streakLog||{}).length;
  S.rpg.rank=rankFromXP(S.rpg.xp||0);
  S.rpg.streak=streakDays;
  const set=(id,val)=>{ const el=$(id); if(el) el.textContent=String(val); };
  set('#phaseBadge',`D${day}`);
  set('#missionLine',`MISSÃO DO MOMENTO: ${mission.title}`);
  set('#hudLevel',S.rpg.level||1);
  set('#hudXP',S.rpg.xp||0);
  set('#hudRank',S.rpg.rank||'Recruta');
  set('#hudInt',S.rpg.integrity??100);
  set('#hudStreak',S.rpg.streak||0);
  set('#hudCombo',S.rpg.combo||0);
  set('#hudBuild',`v${APP_VERSION}`);
}
function renderTabs(){
  if(!tabs) return;
  tabs.innerHTML=TAB_ITEMS.map(t=>`<button class='tab ${activeTab===t.id?'active':''}' data-tab='${t.id}'>${t.label}</button>`).join('');
  tabs.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{
    const next=b.dataset.tab;
    if(S.strictMode && featureOn('strictNavigation') && !['CFG','REL'].includes(next)){
      const win=currentMissionWindow();
      if(next!==win.tab) return showToast(`Modo estrito: vá para ${win.tab}`);
    }
    activeTab=next;
    render();
  });
...
function viewHUD(){
  ...
  const k=todayKey();
  const study=(S.study.history||[]).filter(x=>x.date===k).reduce((a,b)=>a+b.minutes,0);
  const train=(S.training.history||[]).filter(x=>x.date===k).length;
  const diet=sumDiet(ensureDietToday());
  view.innerHTML=`<div class='card'><h2>Painel Diário</h2><div class='kpi'><div><div class='big'>${p.pct}%</div><div class='small'>Pilares concluídos ${p.done}/${p.total}</div></div><span class='badge'>Disciplina</span></div><div class='progress'><div style='width:${p.pct}%'></div></div></div>
  <div class='card'><h2>Resumo de hoje</h2><div class='list'>
    <div class='item'><div><div class='name'>Estudo</div><div class='meta'>Minutos focados</div></div><span class='badge'>${study} min</span></div>
    <div class='item'><div><div class='name'>Treino</div><div class='meta'>Séries registradas</div></div><span class='badge'>${train}</span></div>
    <div class='item'><div><div class='name'>Dieta</div><div class='meta'>Calorias / proteína</div></div><span class='badge'>${diet.kcal} kcal • P ${diet.p}</span></div>
  </div></div>`;
}
function viewProtocolo(){
  const k=todayKey();
  const today=(S.proto.history||[]).filter(x=>x.date===k);
  const doneSet=new Set(today.map(x=>x.item));
  const list=[...(S.proto.itemsMorning||[]),...(S.proto.itemsNight||[])];
  view.innerHTML=`<div class='card'><h2>Protocolo diário</h2><div class='list'>${list.map((item,i)=>`<div class='item'><div><div class='name'>${item}</div></div><button class='btn ${doneSet.has(item)?'ghost':'primary'}' data-proto='${i}'>${doneSet.has(item)?'FEITO':'MARCAR'}</button></div>`).join('')}</div></div>`;
  view.querySelectorAll('[data-proto]').forEach(b=>b.onclick=()=>{ const item=list[Number(b.dataset.proto)]; if(doneSet.has(item)) return; S.proto.history.push({date:k,item}); addXP(6,'proto'); adjustIntegrity(+1); saveState(); render(); });
}
function viewDieta(){
  const day=ensureDietToday();
  const mkSel=(slot,label)=>`<div class='g6'><label>${label}</label><select data-meal='${slot}'>${(MEALS[slot]||[]).map((m,i)=>`<option value='${i}' ${Number(day.meals[slot]||0)===i?'selected':''}>${m.name} (${m.kcal} kcal)</option>`).join('')}</select></div>`;
  const sum=sumDiet(day);
  view.innerHTML=`<div class='card'><h2>Dieta</h2><div class='grid'>${mkSel('cafe','Café')} ${mkSel('almoco','Almoço')} ${mkSel('lanche','Lanche')} ${mkSel('jantar','Jantar')}<div class='g12'><button class='btn primary wide' id='btnDietSave'>Salvar dieta</button></div></div><div class='hint'>Total: ${sum.kcal} kcal • P ${sum.p} • C ${sum.c} • G ${sum.g}</div></div>`;
  $('#btnDietSave').onclick=()=>{ view.querySelectorAll('[data-meal]').forEach(sel=>{ day.meals[sel.dataset.meal]=Number(sel.value); }); addXP(8,'diet'); saveState(); showToast('Dieta salva'); render(); };
}
function fmtTimerSec(sec=0){ return `${String(Math.floor(sec/60)).padStart(2,'0')}:${String(sec%60).padStart(2,'0')}`; }
function getExerciseDefByName(name=''){ const n=String(name).toLowerCase(); return EXERCISES_DB.find(e=>e.name.toLowerCase()===n) || EXERCISES_DB.find(e=>n.includes(e.name.toLowerCase().split(' ')[0])); }
function getMuscle(id){ return MUSCLE_GROUPS.find(m=>m.id===id) || MUSCLE_GROUPS[0]; }
function tempoToSec(t='2-1-2'){ if(typeof t==='number') return t; const parts=String(t).split('-').map(x=>Math.max(0,Number(x)||0)); return parts.reduce((a,b)=>a+b,0)||5; }
function scientificRestSeconds(exDef,lastSet=false){
  const muscle=getMuscle(exDef?.primary_muscle_id||'chest');
  const base=exDef?.type==='compound' ? (muscle.rest_compound_seconds||150) : (muscle.rest_isolation_seconds||75);
  return Math.max(30,Math.round(base + (lastSet?20:0)));
}
function deterministicTTS(exDef, phase='base', deload=false, lastSet=false, moment='pre'){
  const muscle=getMuscle(exDef?.primary_muscle_id||'chest').name;
  if(deload || phase==='deload') return 'Semana estratégica. Técnica perfeita e recuperação.';
  if(moment==='rest') return lastSet ? 'Última série concluída. Respire e registre.' : `Recupera. Próxima série de ${muscle}.`;
  if(moment==='exec') return 'Executa com controle. Sem ego.';
  return `Foco em ${muscle}. Cadência limpa.`;
}
function getProgramAndDay(){
  const track=S.training.program.track||S.training.environment||'home';
  const prog=TRAINING_PROGRAMS[track]||TRAINING_PROGRAMS.home;
  const dayKey=S.training.program.dayKey||prog.split[0];
  const exercises=(prog.days?.[dayKey]||[]).map(e=>({...e,rest:e.rest||90,tempo:e.tempo||'2-1-2'}));
  return {track,prog,dayKey,exercises};
}
function getPhase(week=1){ if(week<=3) return 'base'; if(week===4||week===9) return 'intensification'; if(week===5||week===10) return 'deload'; return 'peak'; }
function phaseConfig(phase='base'){ if(phase==='deload') return {volume_multiplier:0.65,rpe_target:6.5}; if(phase==='intensification') return {volume_multiplier:0.9,rpe_target:8.8}; if(phase==='peak') return {volume_multiplier:1.05,rpe_target:8.5}; return {volume_multiplier:1,rpe_target:8}; }
function getWeeklyMuscleStats(){
  const sets=weeklyTrainingSets(0);
  return MUSCLE_GROUPS.map(m=>({muscle_id:m.id,name:m.name,weekly_sets:sets[m.name]||0,mav_mid:Math.round((m.MAV_min+m.MAV_max)/2),fatigue_factor:m.fatigue_factor||1}));
}
function computePerformanceGlobalScore(){
  const prog=todayProgress().pct;
  const study=Math.min(100,Math.round(((S.study.history||[]).slice(-7).reduce((a,b)=>a+b.minutes,0))/210*100));
  const treino=Math.min(100,Math.round(((S.training.history||[]).slice(-40).length/24)*100));
  const disciplina=Math.round((Math.min(100,S.rpg.integrity||0)+prog)/2);
  const overall=Math.round((study+treino+disciplina)/3);
  return {study,treino,disciplina,overall};
}
function generateDeterministicWorkout(input={}){ return {summary:'deterministic',...input}; }
function shouldAutoDeload(weeklyStats=[]){ return weeklyStats.some(m=>m.weekly_sets>(MUSCLE_GROUPS.find(x=>x.id===m.muscle_id)?.MRV||999)); }
function getMuscleDashboardPayload(){
  const weekly=getWeeklyMuscleStats();
  const muscles=weekly.map((m,idx)=>{
    const def=MUSCLE_GROUPS.find(x=>x.id===m.muscle_id)||MUSCLE_GROUPS[idx]||MUSCLE_GROUPS[0];
    const mavPercent=Math.round((m.weekly_sets/Math.max(1,m.mav_mid))*100);
    const status=mavPercent<70?'subestimulado':mavPercent<=110?'ideal':mavPercent<=130?'alto':'excesso';
    const fatigueScore=Math.min(100,Math.round((m.weekly_sets*def.fatigue_factor)/(Math.max(1,def.MRV)*1.2)*100));
    return {muscle_id:def.id,name:def.name,weekly_sets:m.weekly_sets,mav_min:def.MAV_min,mav_max:def.MAV_max,mrv:def.MRV,mav_percent:mavPercent,status,stimulus_score:Math.min(100,mavPercent),fatigue_score:fatigueScore,trend_4w:{sets:[m.weekly_sets,m.weekly_sets,m.weekly_sets,m.weekly_sets],stimulus:[mavPercent,mavPercent,mavPercent,mavPercent],fatigue:[fatigueScore,fatigueScore,fatigueScore,fatigueScore]},trend_pct:0,priority:(mavPercent<70?'alta':fatigueScore>75?'controle':'média')};
  });
  return {week_start:todayKey(),week_end:todayKey(new Date(Date.now()+6*86400000)),muscles,alerts:muscles.filter(m=>m.mav_percent<70).map(m=>({type:'undertrained',muscle:m.name,message:`${m.name} abaixo de 70% do MAV. Aumente 2 sets semanais.`}))};
}
const ProgramGenerator={
  getProgram(env='home'){ return PERSONAL_TRAINING_DB[env]||PERSONAL_TRAINING_DB.home; },
  getSession(name,env='home'){ const p=this.getProgram(env); return (p.sessions||[]).find(s=>s.name===name) || (p.sessions||[])[0] || {exercises:[]}; },
  applyPhaseVolume(session, volume=1){ if(!session) return session; return {...session,exercises:(session.exercises||[]).map(e=>({...e,sets:Math.max(1,Math.round((e.sets||1)*volume))}))}; },
  targetLoadFrom1RM(oneRM=100,reps=8){ const pct=Math.max(0.5,Math.min(0.9,1-(Math.max(1,reps)-1)*0.02)); return Math.round(oneRM*pct); }
};
function ensureModeChangeValidity(){ if(!S.modeChange?.enabled) return; if(S.modeChange.active && (Date.now()-S.modeChange.active.startAt)>(S.modeChange.durationMin||45)*60000){ S.modeChange.active=null; saveState(); showToast('Modo Change encerrado'); } }
async function forceRefreshApp(){
  ...
  location.href = `${location.pathname}?v=${APP_VERSION}&t=${Date.now()}`;
}
...
const VoiceEngine = {
  StateController:{state:VOICE_STATES.PRE_WORKOUT,set(next){this.state=next;},canSpeak(state,ctx={}){const cfg=S.training?.voice||{}; if(cfg.mode==='silent') return false; if(cfg.mode==='brutal_focus'){ if(state===VOICE_STATES.REST_MID) return false; if([VOICE_STATES.ECCENTRIC,VOICE_STATES.PAUSE,VOICE_STATES.CONCENTRIC].includes(state)&&(ctx.rep||1)%2===0) return false; return true; } if(cfg.frequency==='reduced'&&[VOICE_STATES.ECCENTRIC,VOICE_STATES.PAUSE,VOICE_STATES.CONCENTRIC].includes(state)&&(ctx.rep||1)%2===0) return false; return true;}},
  ScriptLibrary:{
    ...
    BRUTAL_PRE_SET:['Base.','Respira.','Estável.'],
    BRUTAL_ECC:['Controla.'],
    BRUTAL_PAUSE:['Segura.'],
    BRUTAL_CON:['Sobe.'],
    BRUTAL_LAST:['Agora.','Mais.','Fecha.'],
    BRUTAL_REST_START:['Recupera.'],
    BRUTAL_REST_END:['Prepara.'],
    BRUTAL_RPE9:['Não quebra.','Mantém padrão.','Sem ego.'],
  ...
  resolve(state,ctx={}){ const vmode=S.training?.voice?.mode||'hardcore'; const brutalOn=vmode==='brutal_focus' || !!ctx.brutal_focus; if(brutalOn){ if(state===VOICE_STATES.PRE_SET && ctx.exerciseType==='compound') return this.pick(this.ScriptLibrary.BRUTAL_PRE_SET,ctx.exerciseName); if(state===VOICE_STATES.ECCENTRIC) return this.pick(this.ScriptLibrary.BRUTAL_ECC,ctx.exerciseName); if(state===VOICE_STATES.PAUSE) return this.pick(this.ScriptLibrary.BRUTAL_PAUSE,ctx.exerciseName); if(state===VOICE_STATES.CONCENTRIC) return this.pick(this.ScriptLibrary.BRUTAL_CON,ctx.exerciseName); if(state===VOICE_STATES.LAST_REP_PUSH) return this.pick(this.ScriptLibrary.BRUTAL_LAST,ctx.exerciseName); if(state===VOICE_STATES.REST_START) return this.pick(this.ScriptLibrary.BRUTAL_REST_START,ctx.exerciseName); if(state===VOICE_STATES.REST_END) return this.pick(this.ScriptLibrary.BRUTAL_REST_END,ctx.exerciseName); if([VOICE_STATES.MID_SET_PUSH,VOICE_STATES.LAST_REP_PUSH].includes(state) && Number(ctx.rpe||8)>=9) return this.pick(this.ScriptLibrary.BRUTAL_RPE9,ctx.exerciseName); }
...
function startCadenceSetFlow(exObj, opts={}){
  ...
  const autoBrutal=(exObj.type==='compound' && reps<=6);
  if(autoBrutal) opts={...opts, brutal_focus:true};
...
(function init(){
  ...
  const buildEl=$('#buildTag'); if(buildEl) buildEl.textContent=`v${APP_VERSION}`;
  const seenVersion=Number(localStorage.getItem('ascensao_seen_version')||0);
  if(seenVersion < APP_VERSION){
    localStorage.setItem('ascensao_seen_version', String(APP_VERSION));
    setTimeout(()=>showToast(`Atualização aplicada: v${APP_VERSION}`), 250);
  }
